...
def View_Appointment(request):
    ...
    if not view_appointment:
        messages.info(request, "No appointments found.")
...
def Patient_Appointment_Completed(request):
    ...
    if search_query:
        ...
        ) | completed_appointments.filter(
            appointmentnumber__icontains=search_query
        )
    ...
    total_prescriptions = completed_appointments.exclude(prescription__isnull=True).count()
    total_tests = completed_appointments.exclude(recommendedtest__isnull=True).count()
    
    context = {
        'patientdetails1': completed_appointments,
